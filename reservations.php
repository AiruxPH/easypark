...
<body class="bg-car">
  ...
  <div class="container py-5">
    ...
    <form method="post" class="mb-4">
      <div class="form-group">
        ...
        <select name="vehicle_id" id="vehicle_id" class="form-control" onchange="this.form.submit()" required
          <?= $user_has_active_reservation ? 'disabled' : '' ?>>
        ...
        <small class="form-text text-warning">
          ...
          <?php if ($user_has_active_reservation): ?>
            You have a pending, confirmed, or ongoing reservation. You cannot reserve another slot until it is completed
            or cancelled.
          <?php endif; ?>
        </small>
      </div>
    ...
    <?php if ($user_has_active_reservation): ?>
      <div class="alert alert-info mt-3">You have a pending, confirmed, or ongoing reservation. You cannot reserve another
        slot until it is completed or cancelled.</div>
    <?php endif; ?>
    <?php if ($selected_vehicle_id && empty($active_vehicle_ids[$selected_vehicle_id]) && !$user_has_active_reservation): ?>
      <?php if ($current_step === 3): ?>
        ...
        <form method="post" class="bg-dark text-light p-4 rounded">
          ...
          <dl class="row">
            ...
            <dd class="col-sm-8">Slot <?= htmlspecialchars($selected_slot['slot_number']) ?>
              (<?= htmlspecialchars($selected_slot['slot_type']) ?>)</dd>
            <dt class="col-sm-4">Vehicle</dt>
            <dd class="col-sm-8">
              <?php foreach ($vehicles as $veh) {
                if ($veh['vehicle_id'] == $selected_vehicle_id) {
                  echo htmlspecialchars($veh['brand'] . ' ' . $veh['model'] . ' (' . $veh['type'] . ') - ' . $veh['plate_number']);
                  break;
                }
              } ?>
            </dd>
            <dt class="col-sm-4">Start</dt>
            <dd class="col-sm-8"><?= htmlspecialchars($_POST['start_datetime']) ?></dd>
            ...
            <dd class="col-sm-8">₱<?= htmlspecialchars($_POST['price']) ?></dd>
          </dl>
      ...
      <?php elseif ($show_reservation_form && $selected_slot): ?>
        ...
        <form method="post" class="bg-dark text-light p-4 rounded">
          <input type="hidden" name="slot_id" value="<?= $selected_slot['parking_slot_id'] ?>">
          <input type="hidden" name="vehicle_id" value="<?= $selected_vehicle_id ?>">
          <h4>Reserve Slot <?= htmlspecialchars($selected_slot['slot_number']) ?>
            (<?= htmlspecialchars($selected_slot['slot_type']) ?>)</h4>
          <div class="form-group">
      ...
      <?php else: ?>
        ...
        <?php if (count($available_slots) > 0): ?>
          ...
          <?php if ($has_active_reservation): ?>
            <div class="alert alert-info mt-3">You have an active reservation. You cannot reserve another slot until your
              current reservation is completed or cancelled.</div>
          <?php endif; ?>
          <?php
          // Pagination controls
            ...
            <nav aria-label="Slot pagination">
              <ul class="pagination justify-content-center">
                <li class="page-item<?= $page <= 1 ? ' disabled' : '' ?>">
                  <a class="page-link" href="?vehicle_id=<?= $selected_vehicle_id ?>&page=<?= $page - 1 ?>"
    ...
    <?php else: ?>
      ...
      <?php elseif (count($active_vehicle_ids) === count($vehicles)): ?>
        <div class="alert alert-warning">You have no vehicles available for reservation. All your vehicles have a pending,
          confirmed, or ongoing reservation.</div>
      <?php endif; ?>
    <?php endif; ?>
