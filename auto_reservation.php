...
$stmt_ids = $pdo->prepare("SELECT user_id, reservation_id FROM reservations WHERE start_time < :now AND status='pending'");
    ...
    $stmt_cancel = $pdo->prepare(" UPDATE reservations SET status='cancelled' WHERE start_time < :now AND
    status='pending' ");
    $stmt_cancel->execute(['now' => $now]);
    $log .= " Cancelled reservations: " . $stmt_cancel->rowCount() . " \n"; // Notify Cancelled
    require_once 'includes/notifications.php' ; foreach ($to_cancel as $c) { sendNotification($pdo,
    $c['user_id'], 'Booking Cancelled' , 'Your pending reservation (ID: ' . $c['reservation_id']
    . ') was cancelled due to no-show.' , 'warning' , 'bookings.php' ); } // Expire outdated reservations
    $stmt_ids=$pdo->prepare("SELECT user_id, reservation_id FROM reservations WHERE end_time < :now AND status IN
        ('pending', 'confirmed' )"); $stmt_ids->execute(['now' => $now]);
    ...
    $log .= " Expired reservations: " . $stmt_expire->rowCount() . " \n"; // Notify Expired foreach ($to_expire as $e)
            { sendNotification($pdo, $e['user_id'], 'Booking Expired' , 'Your reservation (ID: ' . $e['reservation_id']
            . ') has expired.' , 'info' , 'bookings.php' ); } // Complete finished ongoing reservations // DISABLED: To
            allow "Live Overstay" charging, we do NOT auto-complete ongoing. // Use manual completion or a separate
            overly-long expiry check. // Pay-as-you-go: Calculate and deduct overstay penalties for ongoing reservations
    ...
    foreach ($overstays as $row) {
        ...
        if ($diff > 0) {
            ...
            // Alternative " Differential" Logic: // Calculate what SHOULD be the total amount now. // We need to know
                original duration cost. // Let's assume the user paid correctly for the initial duration. // So any
                extra above that is penalty. // Let's do this: Store the "last charged hour" or just rely on total
                amount. // If we assume `paid_amount` is accurate: // New Total should be: Initial_Cost + (OverHours *
                Rate). // But we don't know Initial_Cost easily in this loop query. // Simplified Approach: // Just
                charge for the *current* hour if not yet charged. // Too risky. // "Pay-as-you-go" Logic
                utilizing 'payments.amount' as the tracker: // 1. Calculate the TOTAL expected cost for the *entire*
                duration (start to NOW). // Start Time needed. // Let's fetch start_time too. $stmt_details=$pdo->
                prepare("SELECT start_time FROM reservations WHERE reservation_id = ?");
                ...
                // `duration` column stores the count (e.g. 2). `duration_type`? Not stored in `reservations` table
                (based on schema).
                ...
                $stmt_base = $pdo->prepare("SELECT amount FROM payments WHERE reservation_id = ? ORDER BY payment_date
                ASC LIMIT 1");
                ...
                // Log transaction
                $pdo->prepare("INSERT INTO coin_transactions (user_id, amount, transaction_type, description) VALUES (?,
                ?, 'payment', 'Overstay Penalty')")->execute([$u_id, -$to_charge]);

                // Record the penalty payment
                $pdo->prepare("INSERT INTO payments (reservation_id, user_id, amount, status, method, payment_date)
                VALUES (?, ?, ?, 'successful', 'coins', NOW())")->execute([$r_id, $u_id, $to_charge]);

                // Notify User
                sendNotification($pdo, $u_id, 'Overstay Penalty', "An overstay penalty of " . number_format($to_charge,
                2) . " coins has been deducted for Reservation #$r_id due to late completion.", 'warning',
                'bookings.php');
                ...
                WHERE start_time <= :now AND end_time> :now
                    ...
                    FROM reservations r
                    WHERE r.end_time < :now AND r.status IN ('expired', 'completed' , 'cancelled' ) ) AND
                        parking_slot_id NOT IN ( SELECT parking_slot_id FROM reservations WHERE start_time <=:now AND
                        end_time> :now
                        AND status IN ('confirmed', 'ongoing')
                        )
                        ");
                        ...
                        // Log Key Stats to DB
                        $summary = "Cron Run: Cancelled(" . $stmt_cancel->rowCount() . "), Expired(" .
                        $stmt_expire->rowCount() . "), Freed(" . $stmt_free->rowCount() . ")";
